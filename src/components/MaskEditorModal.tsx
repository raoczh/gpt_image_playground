...
export default function MaskEditorModal() {
  ...
  return (
    <>
      ...
      {/* Header */}
      <div className="flex-none flex items-center justify-between px-4 py-3 border-b border-border bg-surface z-20">
        <div className="flex items-center gap-3">
          <button onClick={close} disabled={isSaving} className="p-2 -ml-2 text-muted hover:bg-surface-2 rounded-lg transition" title="取消">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
          <div className="relative flex items-center gap-1.5">
            <h2 className="text-sm font-medium text-foreground" id="mask-editor-title">编辑遮罩</h2>
            <button
              type="button"
              ...
              className="flex h-6 w-6 items-center justify-center rounded-full text-subtle transition hover:bg-surface-2 hover:text-foreground"
              aria-label="遮罩编辑说明"
              title="遮罩编辑说明"
            >
            ...
            {showMaskInfo && (
              <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-border bg-surface px-3 py-2 text-xs leading-5 text-foreground shadow-lg">
                <div className="absolute -top-1.5 left-16 h-3 w-3 rotate-45 border-l border-t border-border bg-surface" />
        ...
        <div className="flex items-center gap-2">
          {maskDraft?.targetImageId === imageId && (
            <button onClick={handleRemoveMask} className="flex h-8 items-center gap-1.5 px-4 text-sm font-medium text-white bg-danger hover:bg-danger/90 rounded-lg transition">
              移除遮罩
            </button>
          )}
          <button onClick={handleSave} disabled={!isReady || isSaving} className="flex h-8 items-center gap-1.5 px-4 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-lg disabled:opacity-50 transition">
            {isSaving ? '保存中...' : '保存'}
          </button>
        </div>
      </div>
      ...
      {/* Workspace */}
      <div ref={stageRef} className="flex-1 relative flex items-center justify-center overflow-hidden bg-canvas p-0 pb-[76px] sm:p-6 sm:pb-[100px]" style={{ containerType: 'size' }}>
        {isLoading && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-surface/50 text-sm text-muted">
            正在载入图片...
          </div>
        )}
        <div
          ref={baseFrameRef}
          className="relative max-h-full max-w-full sm:rounded-xl shadow-inner touch-none bg-canvas"
          onWheel={handleWheel}
          style={{
        ...
        <div className="absolute bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 flex items-center justify-center z-20 pointer-events-none w-full px-2 sm:px-4">
          <div className="flex items-center gap-2 sm:gap-4 px-2 sm:px-3 py-1.5 sm:py-2 bg-elevated border border-border rounded-2xl sm:rounded-[1.25rem] shadow-2xl pointer-events-auto">
            <div className="flex items-center gap-1.5 sm:gap-3">
              <div className="flex items-center bg-surface-2 p-1 rounded-xl sm:rounded-[14px]">
                <button
                  className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl transition-all ${tool === 'brush' ? 'bg-surface shadow-sm text-primary' : 'text-muted hover:text-foreground'}`}
                ...
                <button
                  className={`p-2 sm:p-2.5 rounded-lg sm:rounded-xl transition-all ${tool === 'eraser' ? 'bg-surface shadow-sm text-primary' : 'text-muted hover:text-foreground'}`}
              ...
              <div ref={brushSizeControlRef} className="relative flex items-center justify-center">
                <button
                  ...
                  className={`flex items-center justify-center w-10 h-10 sm:w-[46px] sm:h-[46px] rounded-xl sm:rounded-[14px] transition-all border ${showBrushControls ? 'bg-primary/10 border-primary text-primary' : 'bg-surface border-border text-foreground hover:bg-surface-2'}`}
                  disabled={!isReady || isSaving}
                  title="调节笔刷大小"
                >
            ...
            <div className="flex items-center gap-0.5 sm:gap-2 sm:ml-1">
              <button onClick={handleUndo} disabled={!canUndo} className="p-2 sm:p-2.5 text-muted hover:bg-surface-2 hover:text-foreground rounded-lg sm:rounded-xl disabled:opacity-30 transition-all" title="撤销">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 7v6h6" />
                  <path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13" />
                </svg>
              </button>
              <button onClick={handleRedo} disabled={!canRedo} className="p-2 sm:p-2.5 text-muted hover:bg-surface-2 hover:text-foreground rounded-lg sm:rounded-xl disabled:opacity-30 transition-all" title="重做">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 7v6h-6" />
                  <path d="M3 17a9 9 0 0 1 9-9 9 9 0 0 1 6 2.3l3 2.7" />
                </svg>
              </button>
              <div className="w-px h-4 sm:h-5 bg-border mx-1"></div>
              <button onClick={resetViewTransform} disabled={!isReady || isSaving || !isZoomed} className="p-2 sm:p-2.5 text-muted hover:bg-surface-2 hover:text-foreground rounded-lg sm:rounded-xl disabled:opacity-30 transition-all" title="重置视图">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M4 14h6v6"/>
                  <path d="M20 10h-6V4"/>
                  <path d="M14 10l7-7"/>
              ...
              <button onClick={handleClear} disabled={!isReady || isSaving} className="p-2 sm:p-2.5 text-muted hover:bg-surface-2 hover:text-foreground rounded-lg sm:rounded-xl disabled:opacity-30 transition-all" title="清空遮罩">
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M3 6h18"/>
      ...
      {showBrushControls && sliderAnchor && createPortal(
        <div
          ...
          className="fixed z-[100] h-44 w-14 -translate-x-1/2 bg-elevated rounded-xl shadow-xl border border-border"
        ...
        >
          <input
            ...
            className="absolute left-1/2 top-1/2 h-5 w-32 -translate-x-1/2 -translate-y-1/2 -rotate-90 accent-primary cursor-ns-resize"
            disabled={!isReady || isSaving}
          />
        </div>,
        document.body,
