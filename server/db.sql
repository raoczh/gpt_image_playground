...
CREATE TABLE IF NOT EXISTS `tasks` (
  ...
  `raw_response_payload` LONGTEXT NULL COMMENT '结构化的请求/响应记录 JSON（含 request + response + upstream 三段）',
  `raw_image_urls` JSON NULL COMMENT '上游返回的原始图片 URL 列表',
  `params` JSON COMMENT '生成参数 (size, quality, format, etc.)',
  `actual_params` JSON NULL COMMENT 'API 实际响应参数',
  `revised_prompt_by_image` JSON NULL COMMENT 'API 改写后的提示词，按 image id 映射',
  `request_meta` JSON NULL COMMENT '请求元数据快照（endpoint/method/params/model）',
  ...
  `output_image_sizes` JSON NULL COMMENT '输出图片尺寸 [{w,h},...]',
